...
async function generateSingleArticle(index) {
  ...
  // ★ プロンプト修正：レイアウトの使い分けルールを追加し、長文のテーブル化を防止
const prompt = `
あなたは写真とメンタルヘルスケアを組み合わせた新しいライフスタイルを提案するプロのライターです。
「写活✖︎メンタルヘルス✖︎ヘルスケア」や「マインドフルネスとしての写真」「ビジネス✖︎写活」「ストリートスナップ✖︎マーケティング✖︎ビジネス」をテーマにしたブログ記事をMarkdown形式で作成してください。

【厳守事項 - 以下のルールを絶対に守ってください】
1. AIとしての返事や挨拶は一切含めず、記事のコンテンツ（Markdown）のみを4000字程度を守って出力してください。
...
7. 画像や表などのhtmlを1記事に複数必ず用いてリッチコンテンツにすること。

【レイアウトと装飾の使い分けルール（超重要）】
記事を視覚的に読みやすくするため、内容に合わせて以下のレイアウトを厳格に使い分けてください。

1. 【ステップや詳しい解説】
「〜の手順」「〜のコツ」など、文章での詳しい解説が必要な場面では、絶対に表（テーブル）を使わないでください。代わりに「### 1. 〇〇」「### 2. 〇〇」のように小見出しを使って順番に解説してください。
2. 【表（テーブル）の厳格な記法（Markdown表の禁止）】
比較や一覧などで表を作成する場合、レイアウト崩れを防ぐため「|---|」を用いたMarkdownの表は【絶対に使用禁止】とします。
表を出力する際は、必ず以下のようなHTMLタグ（<table>, <tr>, <th>, <td>）を使用して記述してください。
（例）
<table border="1">
  <tr><th>スキルカテゴリ</th><th>具体的な職務・業務</th></tr>
  <tr><td>データ分析</td><td>市場調査、戦略立案</td></tr>
</table>
3. 【箇条書きの禁止事項】
「* 項目: 説明 * 項目: 説明」のように、複数の箇条書きを改行せずに1行に連続して詰め込むことは絶対に行わないでください。
