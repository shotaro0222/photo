...
export default function ImageUploader() {
  ...
  const handleUpload = async (e) => {
    ...
    if (!token.trim()) return alert('GitHub のトークンを入力してください');
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        setStatus(`❌ エラー: ${data.error || 'アップロードできませんでした'}`);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
      ...
      <p className="text-sm text-gray-500 mb-6">
        GitHub に保存し、AI の記事生成素材として利用できるように追加します。
      </p>

      <form onSubmit={handleUpload} className="space-y-5">
        <div>
          <label className="block text-sm font-bold text-gray-700 mb-2">GitHub トークン</label>
          <input
            type="password"
            value={token}
            onChange={(e) => setToken(e.target.value)}
            placeholder="ghp_から始まるトークン"
